...
// Dual-PHY fixture with a real s27ks0641 on PHY 0 and a behavioral
// hyperbus_ram_stub on PHY 1.
//
// This variant is structurally identical to fixture_hyperbus but replaces
// the PHY 1 device models so the controller can be tested in isolation from
// any PHY 1 device-side timing quirks.  If this fixture passes and
// fixture_hyperbus fails, the root cause is specific to the real PHY 1
// HyperRAM device, not the dual-PHY controller logic.

module fixture_hyperbus_phy1_stub #(
...
`endif
    ...
    // Device models
    //
    // PHY 0: real s27ks0641 (identical to fixture_hyperbus.sv)
    // PHY 1: hyperbus_ram_stub — protocol-correct behavioral stub that
    //        stores writes and replays them on reads, with $display debug
    //        output per transaction.
    // ---------------------------------------------------------------

    // PHY 0 — real HyperRAM
    generate
       for (genvar j=0; j<NumChips; j++) begin : hyperrams_phy0
          s27ks0641 #(
            .TimingModel ( "S27KS0641DPBHI020" )
          ) dut (
            .DQ7     ( pad_hyper_dq[0][7]  ),
            .DQ6     ( pad_hyper_dq[0][6]  ),
            .DQ5     ( pad_hyper_dq[0][5]  ),
            .DQ4     ( pad_hyper_dq[0][4]  ),
            .DQ3     ( pad_hyper_dq[0][3]  ),
            .DQ2     ( pad_hyper_dq[0][2]  ),
            .DQ1     ( pad_hyper_dq[0][1]  ),
            .DQ0     ( pad_hyper_dq[0][0]  ),
            .RWDS    ( pad_hyper_rwds[0]   ),
            .CSNeg   ( pad_hyper_csn[0][0] ),
            .CK      ( pad_hyper_ck[0]     ),
            .CKNeg   ( pad_hyper_ckn[0]    ),
            .RESETNeg( pad_hyper_reset[0]  )
          );
       end
    endgenerate

    // PHY 1 — behavioral stub
    generate
       for (genvar j=0; j<NumChips; j++) begin : hyperrams_phy1
          hyperbus_ram_stub #(
            // Defaults match t_latency_access=6, en_latency_additional=1,
            // PHY_TCK=6ns, t_rx_clk_delay_phy1=8 (→ 2 ns).  See stub header
            // for how to tune if the timing model changes.
            .HalfLatencyRead ( 29         ),
            .HalfLatencyWrite( 27         ),
            .DebugName       ( "phy1_stub")
          ) dut (
            .DQ7     ( pad_hyper_dq[1][7]  ),
            .DQ6     ( pad_hyper_dq[1][6]  ),
            .DQ5     ( pad_hyper_dq[1][5]  ),
            .DQ4     ( pad_hyper_dq[1][4]  ),
            .DQ3     ( pad_hyper_dq[1][3]  ),
            .DQ2     ( pad_hyper_dq[1][2]  ),
            .DQ1     ( pad_hyper_dq[1][1]  ),
            .DQ0     ( pad_hyper_dq[1][0]  ),
            .RWDS    ( pad_hyper_rwds[1]   ),
            .CSNeg   ( pad_hyper_csn[1][0] ),
            .CK      ( pad_hyper_ck[1]     ),
            .CKNeg   ( pad_hyper_ckn[1]    ),
            .RESETNeg( pad_hyper_reset[1]  )
          );
       end
    endgenerate

    // SDF annotation — PHY 0 only (stub needs no SDF)
    generate
       for (genvar l=0; l<NumChips; l++) begin : sdf_annotation
          initial begin
             automatic string sdf_file_path = "./models/s27ks0641/s27ks0641.sdf";
             $sdf_annotate(sdf_file_path, hyperrams_phy0[l].dut);
             $display("Mem PHY0 chip %0d", l);
          end
       end
    endgenerate
   ...
   for (genvar i = 0 ; i<NumPhys; i++) begin: pad_gen
    ...
    initial begin
        ...
        fr = $fopen ("axireadvalues_stub.txt","w");
        fw = $fopen ("axiwrotevalues_stub.txt","w");
...
endmodule : fixture_hyperbus_phy1_stub
