...
module hyperbus_cfg_regs #(
    ...
    always_comb begin : proc_comb_read
        ...
        if (sel_reg_mapped) begin
            rfield = {
                crange_q,
                reg_data_t'(cfg_q.t_tx_clk_delay_phy1),
                reg_data_t'(cfg_q.t_rx_clk_delay_phy1),
    ...
    always_comb begin : proc_comb_write
        ...
        if (reg_req_i.valid & reg_req_i.write & sel_reg_mapped) begin
            case (sel_reg)
                ...
                // Per-PHY path-equalization delay registers (dual-PHY only)
                'hb: cfg_d.t_rx_clk_delay_phy1      = (~wmask & cfg_q.t_rx_clk_delay_phy1     ) | (wmask & reg_req_i.wdata);
                'hc: cfg_d.t_tx_clk_delay_phy1      = (~wmask & cfg_q.t_tx_clk_delay_phy1     ) | (wmask & reg_req_i.wdata);
                default: begin
